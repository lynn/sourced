...
def resource(local_path, create=None, url=None, headers={}, serialize=None, deserialize=None, file_action=None, file_open=None, max_age=None):
    if create and url:
    ...
    if not os.path.isfile(local_path) or (max_age and time.time() - os.stat(local_path).st_mtime > pytimeparse.parse(max_age)):
        ...
        if url:
            ...
            artifact = create()
            binary = serialize(artifact) if serialize else artifact
    ...
    binary = fd.read()
    artifact = deserialize(binary) if deserialize else binary
    return artifact
...
def json(local_path, create=None, url=None, headers={}, encoding='utf-8', indent=None):
    return resource(local_path, create=create, url=url, headers=headers,
